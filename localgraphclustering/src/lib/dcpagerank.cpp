/**
 * dcpagerank with C interface. It takes an unweighted and undirected graph with CSR representation
        ...
        bool simplify)
{
    ...
    graph<uint32_t,uint32_t> g(ai[n],n,ai,aj,nullptr, offset,nullptr);

    // Call the pagerank method on the graph to compute the pagerank itself and return the result.
    uint32_t actual_length = g.dcpagerank(alpha, eps, seedids, nseedids, maxsteps, xids_1, xids_2, xlength, values_1,
                                          values_2, simplify);
        ...
        bool simplify)
{
    // Construct a graph object from the sparse adjacency matrix. Notice that for the unweighted calculation, the actual
    // data vector a is not needed - all of the edge connectivity information is given by ai and aj only.
    graph<int64_t,int64_t> g(ai[n],n,ai,aj,nullptr,offset,nullptr);

    // Call the pagerank method on the graph to compute the pagerank itself and return the result.
    int64_t actual_length = g.dcpagerank(alpha, eps, seedids, nseedids, maxsteps, xids_1, xids_2, xlength, values_1,
                                         values_2, simplify);
    return actual_length;
}
        ...
        bool simplify)
{
    // Construct a graph object from the sparse adjacency matrix. Notice that for the unweighted calculation, the actual
    // data vector a is not needed - all of the edge connectivity information is given by ai and aj only.
    graph<uint32_t,int64_t> g(ai[n],n,ai,aj,nullptr,offset,nullptr);
...
vtype graph<vtype,itype>::dcpagerank(double alpha, double eps, vtype* seedids, vtype nseedids,
                                     ...
                                     double* values_2, bool simplify)
{
    ...
    vtype actual_length;
    actual_length = dc_pprgrow(
            alpha, eps, seedids, nseedids, maxsteps, xids_1, xids_2, xlength, values_1, values_2, simplify);
    return actual_length;
}

/** 
 * dc_pprgrow compute the approximate pagerank vector locally on the double cover of the graph.
 *
 * INUPUT:
 ...
 */
template<typename vtype, typename itype>
vtype graph<vtype,itype>::dc_pprgrow(double alpha, double eps,vtype* seedids, vtype nseedids,
                                     vtype maxsteps, vtype* xids_1, vtype* xids_2, vtype xlength, double* values_1,
                                     double* values_2, bool simplify)
{
#ifdef DEBUG
    cout << endl << "Entering dc_ppgrow." << endl;
#endif
    ...
    while(Q.size() > 0 && steps_count < maxsteps){
        ...
        // pushed again.
        if (this_r_map->at(j) >= eps * get_degree_unweighted(j)) {
#ifdef DEBUG
