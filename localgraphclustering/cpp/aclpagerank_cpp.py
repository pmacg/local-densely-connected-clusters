...
# Load the functions
def _setup_acl_pagerank_args(vtypestr, itypestr, fun):
...
_graphlib_funs_aclpagerank64 = _setup_acl_pagerank_args(
    'int64', 'int64', _graphlib.aclpagerank64)
_graphlib_funs_aclpagerank32 = _setup_acl_pagerank_args(
    'uint32', 'uint32', _graphlib.aclpagerank32)
_graphlib_funs_aclpagerank32_64 = _setup_acl_pagerank_args(
...
def aclpagerank_cpp(n, ai, aj, alpha, eps, seedids, maxsteps, xlength=10**7):
    float_type, vtype, itype, ctypes_vtype, ctypes_itype, fun = _get_aclpagerank_cpp_types_fun(
        ai, aj)
    ...
    flag = 0
    actual_length = fun(n, ai, aj, flag, alpha, eps, seedids,
                        nseedids, maxsteps, xids, xlength, values)
    if actual_length > xlength:
        xlength = actual_length
        xids = np.zeros(xlength, dtype=vtype)
        values = np.zeros(xlength, dtype=float_type)
        actual_length = fun(n, ai, aj, flag, alpha, eps,
                            seedids, nseedids, maxsteps, xids, xlength, values)
