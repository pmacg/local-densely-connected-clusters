...
def spectral_clustering(G, ref_nodes,
    ...
    if G.weighted and method not in ["acl_weighted", "l1reg", "l1reg-rand"]:
    ...
    if method == "acl" or method == "acl_weighted" or method == "l1reg" or method == "l1reg-rand":
        p = approximate_pagerank(G, ref_nodes, timeout=timeout, iterations=iterations, alpha=alpha, rho=rho,
                                 epsilon=epsilon, ys=ys, cpp=cpp, normalize=normalize,
                                 normalized_objective=normalized_objective, method=method)
    elif method == "nibble":
